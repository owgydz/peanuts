...
class Lexer:
    ...
    def tokenize(self):
        for line in self.source:
            ...
            self.tokens.extend(
                self.indent_handler.process(line, self.line_number)
            )

            self.scan_line(line.strip())
            self.tokens.append(Token(TokenType.NEWLINE, line=self.line_number))

        self.tokens.extend(
            self.indent_handler.finalize(self.line_number)
        )

        self.tokens.append(Token(TokenType.EOF, line=self.line_number))
        return self.tokens
    ...
    def scan_line(self, line):
        ...
        while i < len(line):
            ...
            if char.isdigit():
                ...
                self.tokens.append(Token(TokenType.NUMBER, number, self.line_number))
                continue
            ...
            if char == '"':
                ...
                while i < len(line) and line[i] != '"':
                    i += 1
                if i >= len(line):
                    raise LexerError("Unterminated string", self.line_number)
                value = line[start:i]
                i += 1
                self.tokens.append(Token(TokenType.STRING, value, self.line_number))
                continue
            ...
            if char == "+":
                ...
                if i + 1 < len(line) and line[i + 1] == "=":
                    self.tokens.append(Token(TokenType.EQUAL_EQUAL, "==", self.line_number))
                    i += 1
                else:
                    self.tokens.append(Token(TokenType.EQUAL, "=", self.line_number))
            elif char == "!":
                if i + 1 < len(line) and line[i + 1] == "=":
                    self.tokens.append(Token(TokenType.BANG_EQUAL, "!=", self.line_number))
                    i += 1
                else:
                    raise LexerError("Unexpected '!'", self.line_number)
            elif char == ">":
                ...
                raise LexerError(f"Unexpected character '{char}'", self.line_number)
